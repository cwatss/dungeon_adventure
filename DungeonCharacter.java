 ...
 * @author Corinne Watson and Emma
...
public abstract class DungeonCharacter {
    ...
    protected static SoundPlayer soundPlayer = new SoundPlayer();
    ...
    private static String genericAttackSoundPath = "regularattack.wav";
     ...
     * @param opponent passes in the current opponent character
     */
    public void attack (final DungeonCharacter opponent) {
        ...
        if (alive()) {
            ...
            if (randDouble <= myChanceToHit) {
                // PLAY SOUND
                soundPlayer.play(genericAttackSoundPath);

                int damage = generateRangedValue(myMinDamage,myMaxDamage);
                System.out.println(getName()+ " attacked " + opponent.getName());
                opponent.substractHitPoints(damage);
            } else {
                System.out.println(getName()+ " attack on " + opponent.getName()+ " missed!");
            }
